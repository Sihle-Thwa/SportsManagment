...
import { FormFieldConfig, FormConfig } from "./FormBuilder.types";
...
export const FormBuilder = ({
	...
	const generateSchema = () => {
		...
		config.fields.forEach((field) => {
			...
			switch (field.type) {
				...
				case "email":
					fieldSchema = z
						.string()
						.email(
							field.validations?.patternMessage || "Invalid email address",
						);
					break;
				case "number":
			...
			if (field.validations) {
				...
				if (
					...
					z.string()._def.typeName ===
						(fieldSchema as z.ZodTypeAny)._def.typeName
				) {
				...
				if (field.type === "number") {
					if (field.validations.min !== undefined) {
						fieldSchema = (fieldSchema as unknown as z.ZodNumber).min(
			...
			schema[field.name] =
				field.required === false ? fieldSchema.optional() : fieldSchema;
		});
